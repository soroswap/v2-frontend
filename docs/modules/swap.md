...
- **`inputDisabled`** on `SwapPanel` (`SwapPanel.tsx:27-28,50,143`) passes straight through to `TokenAmountInput`'s `disabled` prop; nothing else is rendered for it (an explanatory note was tried and removed as confusing — the disabled field is the whole cue). `src/app/page.tsx:222` is its only caller, setting it on the **Buy** panel exactly when `sodax.isSodaxActive` — SODAX only quotes `exact_input`, so the Buy amount can't be typed while a SODAX pair is selected.
